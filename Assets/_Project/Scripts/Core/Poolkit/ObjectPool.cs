...
public class ObjectPool<T> where T : Component, IPoolItem
{
    ...
    public ObjectPool(Func<T> createFunc, Action<T> actionOnAllocate = null, Action<T> actionOnRecycle = null, int initSize = 0, int capacity = 64)
    {
    ...
    public void Recycle(T item)
    {
        ...
        {
            Debug.LogWarning($"[ObjectPool] Attempted to recycle an item that is already in pool: {item.name}", item);
            return;
        }
